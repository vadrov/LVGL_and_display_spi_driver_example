...
void LCD_String_Interpretator(LCD_Handler* lcd, uint8_t *str)
{
	SPI_TypeDef *spi = lcd->spi_data.spi;
	int i;
	while (LCD_GetState(lcd) != LCD_STATE_READY) { } //ждем когда дисплей освободится
					   ...
					   )
{
	...
	//настройка DMA
	if (hdma->dma) {
		DMA_Stream_TypeDef *dma_x = ((DMA_Stream_TypeDef *)((uint32_t)((uint32_t)hdma->dma + 24 * hdma->stream + 16)));
		dma_x->CR &= 0xffff0000; /* Здесь:
		                        	- запрещаем прерывания по некоторым событиям канала передачи tx и режим двойного буфера;
		                        	- инкремент адреса периферии отключен;
		                        	- режим нормальный, а не кольцевой;
		                        	- ширина данных памяти и периферии 1 байт.
		                          */
		if (lcd->data_bus == LCD_DATA_16BIT_BUS) {
			//ширина данных памяти и периферии 2 байта (полуслово)
			dma_x->CR |= (DMA_SxCR_MSIZE_0 | DMA_SxCR_PSIZE_0);
		}
		/* Здесь:
		   - разрешаем прерывание по окончанию передачи;
		   - включаем инкремент адреса памяти;
		   - направление передачи: из памяти в периферию.
		*/
		dma_x->CR |= (DMA_SxCR_TCIE | DMA_SxCR_MINC | DMA_SxCR_DIR_0);
		//запрещаем прерывание по ошибке стека FIFO
